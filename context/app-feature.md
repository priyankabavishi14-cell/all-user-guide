...
### Markdown Editor Page Section Divider
- "Page Section Divider" button in markdown editor toolbar, positioned after the table buttons with a divider
- Clicking inserts a horizontal rule (`---`) at the cursor position, on its own line, separating content sections
- Divider renders as a styled `<hr>` in the live preview and final page output
- Multiple dividers supported on the same page; each divider is independently editable/removable in the editor
- Divider saves correctly with page content; renders after refresh; does not break surrounding markdown formatting
- Works in both Add Page and Edit Page; all existing editor tools, save flow, and layout unchanged
...
- Requirements: Section Divider toolbar button inserts `---` horizontal rule at cursor; renders as `<hr>` in preview and final output; multiple dividers per page supported; no changes to existing editor design or functionality
