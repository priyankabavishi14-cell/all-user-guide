...
export default async function ReaderTypeLivePage({
  params,
}: {
  ...
  try {
    const readerType = await prisma.readerType.findUnique({
      where: { token },
      include: {
        project: true,
        pageSelections: {
    ...
    if (!readerType || readerType.project.slug !== slug || !readerType.project.isActive) {
      notFound()
    }
  ...
  if (projectPages.length > 0 && !project.welcomeScreenEnabled) {
    redirect(`/${slug}/r/${token}/pages/${projectPages[0].slug}`)
  }

  return (
    <FrontendShell project={project} pages={projectPages} readerTypeToken={token}>
